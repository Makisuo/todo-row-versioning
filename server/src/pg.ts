...
async function initPool() {
	console.info("creating global pool")
...
async function withExecutorAndPool<R>(f: (executor: Executor) => R, p: Pool): Promise<R> {
	...
	const executor = async (sql: string, params?: any[]) => {
		try {
			console.info("Running query", sql, params)
	...
	for (let i = 0; i < 10; i++) {
		try {
			...
			try {
				...
				console.info(`caught error ${e} - rolling back`)
				await executor("rollback")
				throw e
			}
		} catch (e) {
			if (shouldRetryTransaction(e)) {
				console.info(`Retrying transaction due to error ${e} - attempt number ${i}`)
				continue
			}
			throw e
		}
